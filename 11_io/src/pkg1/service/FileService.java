...
public class FileService {
	 ...
	 * Linux/Mac: / (슬래시)
	 * 
	 *  -> Java는 모든 운영체제가 호환되기 때문에 경로 표기 방법도 전부 호환!!
	 *  
	 *  최상위 경로(root) : / 또는 \ 의미
	 *  - Windows   : C 또는 D 드라이브
	 *  - Linux/Mac : / , /Users 
	...
	public void test1() {
		...
		// C:/io_test/20240731 폴더 경로의 폴더를 관리하겠다
	 ...
	 */
	public void test2() {
		// 경로 표기 방법 : 윈도우 \, 리눅스 맥은 / (통일감잇게 하나 쓰기)
		// \는 문자열(String)에서 Excape 문자를 위한 키워드이기 때문에
		// "\" 문자열을 나타내고 싶으면 "\\" 두 번 작성해야 한다
		// "\\" == \
		File file = new File("\\io_test\\20240731\\파일생성.txt");
		
		System.out.println("\\io_test\\20240731\\파일생성.txt");
		
		// 지정된 경로 파일이 존재하지 않는다면
		if( !file.exists() ) {
...
//			System.out.println(f.toString());
			...
			// SimpleDateFormat : 간단히 날짜 형식을 지정할 수 있는 기능제공
			// y : year
			// M : month
			// d : day
			// a : am/pm 
			...
			// 파일 유형
			String type = null;
			if(f.isFile()) type = "파일"; // 파일이야? 라고 물어보는 것
			else 		   type = "폴더";
			...
			if(f.isFile()) { // 파일인 경우에만
				...
				if(size < 1) size = 1; // 1KB 미만은 1KB로 표시하겠다 
				
				kb = size + "KB";
			}
			
			// 출력할 문자열               마이너스 붙으면 왼쪽 정렬
