...
public class ArrayEx1 {
	 ...
	 * - 값이 없고 주소를 저장하는 변수
	 * 
	 * - 변수 호출 시 값을 반환하는 게 아닌 
	 * 	저장된 주소로 찾아가서 해당 위치의 배열/객체를 참조한다
	 *  (주소를 찾아가는 것을 포인터라고 부름)
	 * 
	...
	public void method1(){
		// 배열 선언
		int[] arr; // Stack 메모리 영역에 int[](int 배열) 자료형을 참조할 변수 arr을 선언
		
		// 배열 할당(생성)
		arr = new int[4];
		// heap 메모리 영역에 int4칸짜리 배열을 할당(생성)한 후
		// 생성된 배열의 주소를 arr에 대입한다
		...
		//    float : 0.0f / char : '\u0000' / boolean : false  (각 자료형의 기본 값)
		...
		// 2) index별로 값 초기화
		arr[0] = 100;
//		arr[2] = 3.14; 자료 형이 달라서 안 됨!!! int 배열 요소에 double 대입 불가
		arr[1] = 20;
		arr[2] = 99;
		arr[3] = -20;
		...
		for(int i = 0 ; i < 4 ; i++) {
			System.out.printf("arr[%d]의 요소 값 : %d\n ", i, arr[i]);
		}
		
		// --------------------------
		
		/*
		 * Java의 배열은 크기 조절이 안 된다
		*/
...
//		arr[4] = 999;
	...
	/**
	 * 정수 4개를 입력 받아 배열요소에 차례 대로 대입 후 
	 * 저장된 값 한 번에 출력하기
	 */
	public void method2() {
		int[] arr = new int[4]; // 정수 4칸짜리 배열을 생성하여 배열 주소를 arr 변수에 대입
	...
	public void method4() {
		...
		for(int i = 0 ; i < arr.length ; i++) {
			...
			arr[i] = sc.next().charAt(0); // 입력된 문자열의 0번째 문자를 char로 반환
			// char 입력 방법
